...
class TestGetModelPathCacheHit:
    """Tests for get_model_path() when model file already exists on disk."""

    def test_returns_local_path_when_model_exists(self, tmp_path):
        """get_model_path should return the full local path when .pt file exists."""
        ...
        result = provider.get_model_path("ru", "v5_5_ru")

        assert result == str(model_file)

    def test_returns_path_for_any_language(self, tmp_path):
        """get_model_path should work for any language in the registry."""
        models_dir = tmp_path / ".models" / "silero"
        ...
        provider = SileroTTSModelProvider(models_dir=models_dir)
        result = provider.get_model_path("en", "v3_en")

        assert result == str(model_file)

    def test_does_not_make_network_request_on_cache_hit(self, tmp_path):
        """get_model_path should not fetch models.yml when model is cached."""
        models_dir = tmp_path / ".models" / "silero"
        ru_dir = models_dir / "ru"
        ru_dir.mkdir(parents=True)
        (ru_dir / "v5_5_ru.pt").write_bytes(b"fake model data")

        provider = SileroTTSModelProvider(models_dir=models_dir)
        provider.get_model_path("ru", "v5_5_ru")

    def test_creates_lang_dir_if_missing(self, tmp_path, monkeypatch):
        """get_model_path should create the language directory if missing."""
...
class TestGetModelPathModelsYmlDownload:
    """Tests for get_model_path() downloading models.yml from GitHub."""

    def test_downloads_models_yml_when_missing(self, tmp_path, monkeypatch):
        """get_model_path should download models.yml from GitHub when not on disk."""
        import urllib.request
...
class TestGetModelPathMalformedYml:
    """Tests for get_model_path() error handling."""

    def test_raises_on_malformed_models_yml(self, tmp_path, monkeypatch):
        """get_model_path should raise TTSProcessingError with delete hint for bad YAML."""
        import urllib.request
    ...
    def test_raises_when_model_url_missing_from_registry(self, tmp_path, monkeypatch):
        """get_model_path should raise TTSProcessingError when model not in registry."""
        import urllib.request
    ...
    def test_raises_for_jit_only_model_without_package(self, tmp_path):
        """get_model_path should reject models that only have jit key (no package)."""
        models_dir = tmp_path / ".models" / "silero"
        ru_dir = models_dir / "ru"
        ru_dir.mkdir(parents=True)
...
"""
        ...
        with pytest.raises(TTSProcessingError) as exc_info:
            provider.get_model_path("ru", "aidar_8khz")

        assert "aidar_8khz" in str(exc_info.value.message)


class TestGetModelPathDownload:
    """Tests for get_model_path() downloading .pt model files."""

    def test_downloads_model_directly_to_final_path(self, tmp_path, monkeypatch):
        """get_model_path should download .pt directly to final path (no temp file)."""
        models_dir = tmp_path / ".models" / "silero"
        ru_dir = models_dir / "ru"
        ru_dir.mkdir(parents=True)
...
"""
        ...
        assert result == str(ru_dir / "v5_5_ru.pt")
